...
ll zeroCount(int idx,vector<int>&vec,bool check_either_restricted,bool check_first_zero,int no_of_zeros)
{
    ...
    for(int i=0;i<=k;i++)
    {
        ...
        if(check_first_zero && i==0)
            temp_first_zero=true;
        else
            temp_first_zero=false;
        if(i==0 && temp_first_zero==false)
            temp_zeros++;
        if(i==0 && temp_first_zero==true && vec.size()-1==idx)
            temp_zeros++;
